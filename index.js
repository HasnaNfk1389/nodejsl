...
const supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImR2aGJrcm1jb3JhbGN1dmtwb3loIiwicm9sZSI6ImFub24iLCJpYXQiOjE2OTg4MTY0OTEsImV4cCI6MjAxNDM5MjQ5MX0.EVm69J6eHvVXksf0MpuYk_RtL8EWgsYRVtBage2fAjY';
...
app.post("/materi", upload.single('file'), async (req, res) => {
  const { nama_materi, user_id, deskripsi_materi } = req.body;
  ...
  try {
    ...
    // Insert other form data along with the file URL into the "materi" table
    const { data: newMateri, error: newMateriError } = await supabase
      ...
      .insert({
        ...
        user_id,
        deskripsi_materi,
        file: fileData.Key, // Save the file URL in the database
      })
      .select("*");

    if (newMateriError) {
      console.error('Error inserting data into "materi" table:', newMateriError);
      return response(500, null, "Internal Server Error", res);
    }
